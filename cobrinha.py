...
cria_comida = lambda esp, pos: round(random.randrange(esp, pos - 10) / 10) * 10
...
def jogo ():
    ...
    comida_x = cria_comida(0, largura)
    comida_y = cria_comida(30, altura)

    while not game_over:
        while fechar:
            dis.fill(branco)
            message("Game Over", vermelho,[250, 60])
            message("Perte J para jogar novamento ou S para sair", preto, [50,100])
            score(tamanho)
            display.update()
